...
long vitessek;         // représentant le calcul de la vitesse actuelle
long vitessek_1;       // représentant le calcul de la vitesse passée 
...
float consigneV;        // réprésente la consigne en upas par milliseconde 
float consigneP;        // représente la consigne en nombre de pas 
float consigneP_k_1;    // représente la consigne passée en nombre de pas 
...
void ConversionConsigneVenConsigneP(void);  // convertir la consigne vitesse en consigne position 
...
//----------------------------------------------------------------------------//
//-- nom 				: ConversionConsigneVenConsigneP
//-- entrée - sortie 	: - / - 
//-- description 		: Machine d'état définissant la gestion de saut sur 5s
//-- remarque 			: 
//-- modification       : 28.01.2021
//----------------------------------------------------------------------------//
void ConversionConsigneVenConsigneP(void)
{
    consigneP = consigneV + consigneP_k_1; 
}
...
int main(int argc, char** argv) 
{
    ...
    while(1)
    {
        ...
        if (temps_1ms > tempPID)
        {
            ...
            CalculerVitesse();
            //-- caclul de la consigne
            ConversionConsigneVenConsigneP(); 
        ...
        OC1RS = (uint16_t)((((float)vitessek / 1048.44) * 3685.0) + 1842.0);                    
        //OC1RS = (((vitessek / (10484.0/10.0)) * 3685) + 1842); 
        //- Pulse 
        OC2RS = (uint16_t)(((consigneV / 1048.44) * 3685.0) + 1842.0); 
...
void CalculerErreur()
{
    //erreurk = consigneV - (float)vitessek;   // sans l'intégrateur en z 
    erreurk = consigneP - posk;                // avec l'intégrateur en z 
}
